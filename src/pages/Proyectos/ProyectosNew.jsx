...
const ProyectosNew = () => {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 max-w-full overflow-x-hidden">
      <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900 dark:text-gray-100 px-2 sm:px-0">Nuevo Proyecto</h2>
      <form onSubmit={handleSubmit} className="bg-blue-50 dark:bg-blue-100/10 rounded-lg border border-blue-200 dark:border-blue-700 p-3 sm:p-4 md:p-6 space-y-4 sm:space-y-6 mx-2 sm:mx-0">
        ...
        <div className="space-y-2 sm:space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-3">
            {['tipo8','tipo10','tipo12','tipo14','tipo15','tipo16','tipo20'].map((k) => (
              <Input key={k} label={k} name={k} type="number" value={formData.cable[k]} onChange={(e)=>handleNestedChange('cable', k, e.target.value)} className="text-xs sm:text-sm" />
            ))}
          </div>
        </div>
        ...
        <div className="space-y-2 sm:space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-3">
            {['tipo8','tipo10','tipo12','tipo14','tipo15','tipo16','tipo20'].map((k) => (
              <Input key={k} label={k} name={k} type="number" value={formData.caja_empalme[k]} onChange={(e)=>handleNestedChange('caja_empalme', k, e.target.value)} className="text-xs sm:text-sm" />
            ))}
          </div>
        </div>
        ...
        <div className="space-y-2 sm:space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-3">
            {['tipo8','tipo10','tipo12','tipo14','tipo15','tipo16','tipo20'].map((k) => (
              <Input key={k} label={k} name={k} type="number" value={formData.reserva[k]} onChange={(e)=>handleNestedChange('reserva', k, e.target.value)} className="text-xs sm:text-sm" />
            ))}
          </div>
        </div>
        ...
        <div className="space-y-2 sm:space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-3">
            {['tipo8','tipo10','tipo12','tipo14','tipo15','tipo16','tipo20'].map((k) => (
              <Input key={k} label={k} name={k} type="number" value={formData.nap[k]} onChange={(e)=>handleNestedChange('nap', k, e.target.value)} className="text-xs sm:text-sm" />
            ))}
          </div>
        </div>
        ...
        <div className="space-y-2 sm:space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-3">
            {['tipo8','tipo9','tipo10','tipo11','tipo12','tipo14','tipo16'].map((k) => (
              <Input key={k} label={k} name={k} type="number" value={formData.altura_final_poste[k]} onChange={(e)=>handleNestedChange('altura_final_poste', k, e.target.value)} className="text-xs sm:text-sm" />
            ))}
          </div>
        </div>

        <div className="flex flex-col sm:flex-row gap-2 sm:gap-4">
          <Button type="submit" variant="primary" disabled={saving} className="w-full sm:w-auto text-xs sm:text-sm">{saving ? 'Guardando...' : 'Crear Proyecto'}</Button>
